...
export default function VendorOwnerCard({ owner, navigation }) {
  const { deleteOwner } = useContext(VendorContext);
  ...
  return (
    <View key={owner.id} style={styles.ownerCard}>
      ...
      <View style={styles.main}>
        ...
        {owner.governmentId?.uri && (
          <View style={{ marginTop: 2,marginBottom:2 }}>
            <View style={styles.infoRow}>
                            <Icon name="document-text-outline" size={16} color={appColors.secondary} style={{ marginRight: 8 }} />
            <Text style={[styles.infoText, { marginBottom: 6 }]}>Government ID</Text>

              </View>
            <Image
              source={{ uri: owner.governmentId.uri }}
              style={{ width: 200, height: 100, borderRadius: 8, resizeMode: 'cover' }}
            />
          </View>
        )}
      </View>
