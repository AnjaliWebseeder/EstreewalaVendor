...
import { getCompletionStatus } from '../../redux/slices/vendorOnboardingSlice';

const SubscriptionPlansScreen = ({ navigation, route }) => {
  const dispatch = useDispatch();
  const { plans, loading, creating, verifying, error, currentOrder } =
  ...
  const initiateRazorpayPayment = async () => {
    ...
    try {
      ...
      console.log('🟢 RAW Razorpay Response:', JSON.stringify(razorpayResponse, null, 2));
  ...
  //     };
  //     const result = await dispatch(
  //       verifySubscription(verificationData),
  //     ).unwrap();
  //     if (result.success) {
  //       setShowPaymentModal(false);
  ...
  //       }
  //       // ✅ ALWAYS go to VendorRegistration
  //       // navigation.reset({
  //       //   index: 0,
  ...
  const handlePaymentVerification = async razorpayResponse => {
    try {
      ...
      const statusResult = await dispatch(getCompletionStatus()).unwrap();
      console.log('🟢 Completion status:', statusResult);
  
      const completionStep = statusResult?.completionStep ?? 0;
      ...
      showToast('Something went wrong while activating subscription', 'error');
    } finally {
      setIsPaying(false);
      setShowPaymentModal(false);
    }
          ...
          <View style={styles.faqGrid}>
            <View style={styles.faqCard}>
              ...
              <Text style={styles.faqAnswer}>
                Immediately after payment confirmation with instant access to
                all features.
              </Text>
            </View>
            ...
            <View style={[styles.faqCard, { width: '40%' }]}>
              ...
              <Text style={styles.faqAnswer}>
                Yes! Upgrade or downgrade anytime. Changes apply to the next
                billing cycle.
              </Text>
            </View>
          </View>
        ...
        <View style={[styles.footer, { paddingBottom: insets.bottom + 10 }]}>
          ...
          <TouchableOpacity
            ...
            style={[
              ...
              (!selectedPlan || creating) && { opacity: 0.6 },
            ]}
            onPress={handleSubscribe}
                ...
                <View style={styles.modalHeader}>
                  <View style={styles.modalTitleContainer}>
                    <View
                      style={[
                        styles.planIcon,
                        { backgroundColor: selectedPlanDetails.color },
                      ]}
                    >
                      <Icon
                        name={selectedPlanDetails.icon}
                        size={24}
                        color="#FFFFFF"
                      />
                    </View>
                      ...
                      <View key={duration.code} style={styles.pricingRow}>
                        ...
                        <View style={styles.pricingRight}>
                          <Text style={styles.pricingAmount}>
                            ₹
                            {
                              selectedPlanDetails.apiData?.durations?.[
                                duration.code
                              ]?.price
                            }
                          </Text>
          ...
          <View style={[styles.modalContent, styles.paymentModalContent]}>
            ...
            <View style={styles.paymentPlanInfo}>
              ...
              <Text style={styles.paymentValidity}>
                {currentOrder?.planDetails?.validityDays ||
                  durations.find(d => d.code === selectedDuration)?.days}{' '}
                days validity
              </Text>
            </View>
            ...
            {currentOrder?.planDetails?.note && (
              <View style={styles.note}>
                <Text
                  style={[styles.paymentFeatureText, { fontWeight: '700' }]}
                >
                  Note:{' '}
                  {currentOrder?.planDetails?.note
                    ?.replace('Prorated:', '')
            ...
            >
              <View style={styles.razorpayButtonContent}>
                ...
                <Text style={styles.razorpayButtonText}>
                  {isPaying ? 'Processing...' : `Pay ₹${currentOrder?.amount}`}
                </Text>
              </View>
